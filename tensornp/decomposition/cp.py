'''
@author: duke
@Date: 2021-04-26 21:17:15
@LastEditTime: 2021-06-03 09:53:52
@LastEditors: duke
@Description: CaNDECOMP/PaRaFaC Decomposition
'''
from .. import backend as T
import numpy as np
from ..linalg import norm, mttkrp, fold, seq_kr
import logging
...
def reconstruct_cp(factors, lamda, shape):
    ...
    rec_tensor = fold(T.dot(factors[0], seq_kr(factors, exclude=0, reverse=False).T), 
                      0, shape)
    return rec_tensor
...
def rec_error_calc(tensor, lamda, norm_tensor, factors, mttkrp=None):
    ...
    if mttkrp is not None:
        ...
        factors_norm = T.sqrt(coef.sum())
        
        # mttkrp and factor for the last mode. This is equivalent to the
        # inner product <tensor, factorization>
        iprod = T.sum(T.sum(mttkrp * factors[-1], axis=0))
        unnorml_rec_error = T.sqrt(T.abs(norm_tensor**2 + factors_norm**2 - 2 * iprod))
    else:
...
def cp(tensor: np.ndarray, r: int, stop_iter=0, init='random', normalize_factor=True, 
    ...
    if random_seed: 
        T.seed(seed=random_seed)
        
    norm_tensor = norm(tensor, 2)
    rec_errors = []
    order = tensor.ndim
    factors = [None] * order
    lamda = T.ones(order).reshape(1, -1)

    # Initialize factor matrix 
    for i in range(order):
        factors[i] = T.rand(tensor.shape[i], r)   
    ...
    for iteration in range(stop_iter):
        for i in range(order):
            ...
            factors[i] = mtt @ T.pinv(v)
            if normalize_factor:
                scales = norm(factors[i], 2, axis=0)
                lamda = T.where(scales==0, T.ones(T.shape(scales)), scales)
